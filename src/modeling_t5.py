...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config, opt):
        ...
        self.add_fusion = opt.add_fusion
    ...
    def forward(
        ...
        if self.add_loss == 'binary':
            ...
            #     max_values = max_values / torch.sum(max_values, dim=1)[:, None]

            #     weighted_sum = torch.sum(t * max_values[:, :, None, :], dim=2)
            #     return weighted_sum
            ...
            # def quantum_fusion_sum(t):
            #     bs, n_passage, max_length, dimension = t.shape

            #     # [400, 200, 768]    
            #     t = t.reshape(bs * n_passage, max_length, dimension)

            #     rets = []
            #     for i in range(t.shape[0]):
            #         psi_real = t[i]
            #         psi_imag = self.Linear(psi_real)
            #         psi = psi_real * torch.exp(1j * psi_imag)

            #         max_values, _ = torch.max(psi_real, dim=1)
            #         max_values = max_values / torch.sum(max_values)

            #         sum_psi = torch.sum(max_values[:, None] * psi, dim=0)
            #         # average_psi = torch.mean(psi, dim=0)
            ...
            def quantum_fusion_ds(t):
                bs, n_passage, max_length, dimension = t.shape
                rs_t = t.reshape(bs * n_passage, max_length, dimension)  # [200, 200, 768]
                amplitudes = self.Linear_a(rs_t)
                phases = self.Linear_p(rs_t)
                rets = []
            
                p = amplitudes * torch.exp(1j * phases)  # [200, 200, 768]
                pT = p.transpose(1, 2)  # [200, 768, 200]
                lo = torch.matmul(p, pT)  # [200, 200, 200]
            ...
            def quantum_fusion_ds_1(t):
                bs, n_passage, max_length, dimension = t.shape
                rs_t = t.reshape(bs * n_passage, max_length, dimension)  # [200, 200, 768]
                # amplitudes = self.Linear_a(rs_t)
                # phases = self.Linear_p(rs_t)
                amplitudes = rs_t.clone()
                phases = rs_t.clone()
                rets = []
            ...
            def quantum_fusion_ds_2(t):
                ...
                p = amplitudes + 1j * phases  # [200, 200, 768]
                pT = p.transpose(1, 2)  # [200, 768, 200]
                lo = torch.matmul(p, pT)  # [200, 200, 200]
                for psi, l in zip(rs_t, lo):
                    eigenvalues = torch.diag(l)
            ...
            def quantum_fusion_ds_3(t):
                ...
                rets = []
                # z=r(cosθ+i sinθ)
                p = amplitudes * (torch.cos(phases) + 1j * torch.sin( phases))  # [200, 200, 768]
                pT = p.transpose(1, 2)  # [200, 768, 200]
                lo = torch.matmul(p, pT)  # [200, 200, 200]
                for psi, l in zip(rs_t, lo):
                    eigenvalues = torch.diag(l)
            ...
            elif self.add_fusion == 'qf_ds':
                encoder_last_hidden_state = quantum_fusion_ds_v3(encoder_last_hidden_state)
            elif self.add_fusion == 'qf_ds_1':
                encoder_last_hidden_state = quantum_fusion_ds_v3_1(encoder_last_hidden_state)
            elif self.add_fusion == 'qf_ds_2':
                encoder_last_hidden_state = quantum_fusion_ds_v3_2(encoder_last_hidden_state)
            elif self.add_fusion == 'qf_ds_3':
                encoder_last_hidden_state = quantum_fusion_ds_v3_3(encoder_last_hidden_state)
            else:
